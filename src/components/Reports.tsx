...
import { 
  ShoppingBag, PhilippinePeso, Download, 
  RefreshCw, Calendar, ChevronRight, Clock, TrendingUp 
} from "lucide-react";
...
export const Reports = () => {
  ...
  const busyHours = Array.from(hourCounts.entries())
    ...
    .map(([hour, count]) => ({
      hour,
      display: hour === 0 ? '12 AM' : hour === 12 ? '12 PM' : hour > 12 ? `${hour - 12} PM` : `${hour} AM`,
      count
    }));
  ...
  return (
    <div className="bg-cream min-h-screen text-dark-brown font-quicksand">
      <div className="fixed top-0 left-0 h-screen w-64 z-10"><Sidebar /></div>
        ...
        <div className="max-w-7xl mx-auto">
              ...
              <div className="relative">
                ...
                {showDatePicker && (
                  <div className="absolute right-0 mt-2 bg-white border rounded-xl shadow-lg z-20 w-48">
                    <button onClick={() => setDatePreset(1)} className="w-full text-left px-4 py-2 hover:bg-gray-50">Today</button>
                    <button onClick={() => setDatePreset(7)} className="w-full text-left px-4 py-2 hover:bg-gray-50">Last 7 days</button>
                    <button onClick={() => setDatePreset(30)} className="w-full text-left px-4 py-2 hover:bg-gray-50">Last 30 days</button>
                  </div>
                )}
              </div>
              <button 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-sm text-gray-400 uppercase tracking-wide">Today's Sales</p>
                  <p className="text-2xl font-black mt-1">{todayCount} orders</p>
                  <p className="text-lg font-bold text-brown-two">₱{todayRevenue.toLocaleString()}</p>
                </div>
                <div className="w-12 h-12 bg-brown/10 rounded-xl flex items-center justify-center">
          ...
          <div className="grid lg:grid-cols-2 gap-6 mb-6">
            {/* Top Drinks */}
            <div className="bg-white rounded-2xl p-6 shadow-sm border">
              <h3 className="font-bold text-lg mb-4">🥤 Top Selling Drinks</h3>
              <div className="space-y-3">
                  ...
                  <div key={name} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                        ...
                        i === 2 ? 'bg-amber-600 text-white' : 'bg-brown/20 text-brown'
                      }`}>
                        {i + 1}
                      </div>
                ...
                {topDrinks.length === 0 && <p className="text-gray-400 text-center py-4">No orders yet</p>}
              </div>
            </div>

            {/* Popular Toppings */}
            <div className="bg-white rounded-2xl p-6 shadow-sm border">
              <h3 className="font-bold text-lg mb-4">✨ Popular Toppings</h3>
              <div className="flex flex-wrap gap-2">
                {topToppings.map(([name, count]) => (
                  <div key={name} className="flex flex-col items-center p-3 bg-brown/5 rounded-xl min-w-[80px]">
                    <span className="font-medium text-dark-brown">{name}</span>
                    <span className="text-xs text-gray-500 mt-1">{count} orders</span>
                  </div>
                ))}
                {topToppings.length === 0 && <p className="text-gray-400 text-center py-4 col-span-full">No toppings added yet</p>}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border mb-6">
            <h3 className="font-bold text-lg mb-4">⏰ Busiest Hours</h3>
            {busyHours.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {busyHours.map(({ display, count }, i) => (
                  <div key={display} className="text-center p-4 bg-brown/5 rounded-xl">
                    ...
                    {i === 0 && <p className="text-xs text-green-600 mt-1">Peak hour</p>}
                  </div>
                ))}
              </div>
            ) : (
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border">
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-gray-400">No orders in this period</div>
            ) : (
                  ...
                  <tbody>
                    {orders.slice(0, 10).map((order: Order) => (
                      <tr key={order.id} className="border-b last:border-0">
                        ...
                        <td className="py-3 text-gray-600 max-w-md truncate" title={order.order_details}>
                          {order.order_details.length > 40 ? order.order_details.slice(0,40) + '...' : order.order_details}
                        </td>
                        <td className="py-3">
                          <span className={`px-2 py-1 rounded-full text-xs capitalize ${
                            order.status === 'completed' ? 'bg-green-100 text-green-700' : 
                            order.status === 'preparing' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'
                          }`}>
                            {order.status}
                          </span>
                        </td>
                        <td className="py-3 font-medium">₱{order.total_price?.toFixed(2) || '0.00'}</td>
